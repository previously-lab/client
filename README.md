...
一条命令完成：找到 agent 仓库 → 构建其 standalone 内核产物 → 构建本 CLI → 委托 `previously init --non-interactive --skip-ingest` 初始化/审计 `~/.previously`（首次运行探测本机 agent CLI，优先配置为纯订阅 bridge 大脑；每次运行都会修复损坏配置）→ 安装新构建的内核 → `previously start` 启动服务并 `previously open` 打开浏览器。
