...
/**
 * True when either timeline source exists (timeline.md or the JSON index).
 * The top-level timeline.md is generated by the kernel during chat-turn
 * housekeeping — after init/ingest but before the first chat round neither
 * source legitimately exists, which callers may want to treat as "not yet"
 * rather than an error.
 */
export function hasTimeline(memoryRoot: string): boolean {
  return (
    existsSync(join(episodicDir(memoryRoot), 'timeline.md')) ||
    existsSync(join(episodicDir(memoryRoot), 'timeline', 'index.json'))
  );
}
