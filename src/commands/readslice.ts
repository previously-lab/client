...
import { MemoryError, readSlice } from '../lib/memory.js';
import { resolvePaths } from '../lib/paths.js';

/**
 * `previously readslice <sliceId> [--start N --end N]` — the constrained
 * slice reader for bridged client agents (phase outsourcing). Prints the
 * slice's conversation record (timeline/core.md), optionally narrowed to a
...
function parseArgs(args: string[]): ParsedArgs {
  ...
  if (sliceId === undefined || positional.length > 1) {
    throw new Error('Usage: previously readslice <sliceId> [--start N --end N]');
  }
...
export async function run(args: string[]): Promise<number> {
  ...
  try {
    const content = readSlice(memoryRoot, parsed.sliceId, {
